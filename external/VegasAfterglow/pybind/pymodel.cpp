...
MediumVariant PyWind(Real A_star, std::optional<Real> n_ism_opt, std::optional<Real> n0_opt, Real k_m) {
    ...
    return Wind(A_star, n_ism / unit::cm3, n0 / unit::cm3, k_m);
}

MediumVariant PySmoothBrokenPowerLaw(Real n_t, Real r_t, Real k1, Real k2, Real s_n, Real X_h) {
    if (s_n == 0) {
        throw std::invalid_argument("SmoothBrokenPowerLaw: s_n must be non-zero");
    }
    return SmoothBrokenPowerLaw(
        n_t / unit::cm3, // cm^-3 -> internal
        r_t * unit::cm,  // cm -> internal
        k1,
        k2,
        s_n,
        X_h);
...
void PyModel::single_evo_details(Shock const& shock, Coord const& coord, Observer& obs, PyRadiation const& rad,
    ...
    auto syn_ph = generate_syn_photons(shock, syn_e, coord, medium_, rad.rad.smooth_fast_to_slow_transition);
...
auto PyModel::sky_image(PyArray const& t_obs, double nu_obs, double fov, size_t npixel) -> PySkyImage {
    ...
    auto render_shock_frames = [&](Shock const& shock, Coord const& coord, PyRadiation const& rad) {
        ...
        auto syn_e = generate_syn_electrons(shock, coord);
        auto syn_ph =
            generate_syn_photons(shock, syn_e, coord, medium_, rad.rad.smooth_fast_to_slow_transition);

        if (rad.ssc) {
            if (rad.kn) {
                KN_cooling(syn_e, syn_ph, shock, coord, medium_, rad.rad.smooth_fast_to_slow_transition,
                           obs_setup.z);
            } else {
                Thomson_cooling(syn_e, syn_ph, shock, coord, medium_, rad.rad.smooth_fast_to_slow_transition,
                                obs_setup.z);
