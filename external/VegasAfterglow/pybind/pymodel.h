...
/**
 * <!-- ************************************************************************************** -->
 * @brief Creates a native smoothly broken power-law external medium profile.
 * @param n_t Number density at transition radius r_t [cm^-3]
 * @param r_t Transition radius [cm]
 * @param k1 Inner density power-law index
 * @param k2 Outer density power-law index
 * @param s_n Smoothness parameter (must be non-zero)
 * @param X_h Hydrogen mass fraction used for number->mass conversion (default 0.7)
 * @return MediumVariant Native isotropic smooth-broken medium
 * <!-- ************************************************************************************** -->
 */
MediumVariant PySmoothBrokenPowerLaw(Real n_t, Real r_t, Real k1, Real k2, Real s_n, Real X_h = 0.7);
...
class PyRadiation {
  public:
    ...
    PyRadiation(Real eps_e, Real eps_B, Real p, Real xi_e = 1, bool ssc = false, bool kn = false,
                bool cmb_cooling = false, bool smooth_fast_to_slow_transition = false)
        : rad(RadParams{eps_e, eps_B, p, xi_e, cmb_cooling, smooth_fast_to_slow_transition}), ssc(ssc), kn(kn) {}
    ...
    [[nodiscard]] std::string repr() const {
        ...
        if (rad.smooth_fast_to_slow_transition) {
            s += ", smooth_fast_to_slow_transition=True";
        }
        s += ")";
        return s;
    }
  ...
  public:
    ...
    [[nodiscard]] std::string repr() const {
        ...
        snprintf(buf, sizeof(buf),
                 ...
                 "      fwd_rad=Radiation(eps_e=%.6g, eps_B=%.6g, p=%.6g%s%s%s%s)",
                 ...
                 fwd_rad.rad.cmb_cooling ? ", cmb_cooling=True" : "",
                 fwd_rad.rad.smooth_fast_to_slow_transition ? ", smooth_fast_to_slow_transition=True" : "");
        std::string s = buf;
        if (rvs_rad_opt) {
            snprintf(buf, sizeof(buf), ",\n      rvs_rad=Radiation(eps_e=%.6g, eps_B=%.6g, p=%.6g%s%s)",
                     rvs_rad_opt->rad.eps_e, rvs_rad_opt->rad.eps_B, rvs_rad_opt->rad.p,
                     rvs_rad_opt->rad.cmb_cooling ? ", cmb_cooling=True" : "",
                     rvs_rad_opt->rad.smooth_fast_to_slow_transition ? ", smooth_fast_to_slow_transition=True" : "");
            s += buf;
        }
...
void PyModel::single_shock_emission(Shock const& shock, Coord const& coord, Array const& t_obs, Array const& nu_obs,
    ...
    auto syn_ph = [&] {
        ...
        return generate_syn_photons(shock, syn_e, coord, medium_, rad.rad.smooth_fast_to_slow_transition);
    }();

    if (rad.ssc) {
        AFTERGLOW_PROFILE_SCOPE(cooling);
        if (rad.kn) {
            KN_cooling(syn_e, syn_ph, shock, coord, medium_, rad.rad.smooth_fast_to_slow_transition, obs_setup.z);
        } else {
            Thomson_cooling(syn_e, syn_ph, shock, coord, medium_, rad.rad.smooth_fast_to_slow_transition,
                            obs_setup.z);
        }
    } else if (rad.rad.cmb_cooling) {
        AFTERGLOW_PROFILE_SCOPE(cooling);
        CMB_cooling(syn_e, syn_ph, shock, coord, medium_, rad.rad.smooth_fast_to_slow_transition, obs_setup.z);
    }
