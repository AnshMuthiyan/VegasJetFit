...
Real estimate_t_dec(Ejecta const& jet, Medium const& medium, Real phi, Real theta) {
    ...
    if constexpr (HasMass<Medium>) {
        const Real m_min = medium.mass(r_min);
        if (m_min >= target) {
            return r_min * (1 - beta) / (beta * con::c);
        }
        ...
        // Bracket root of m(r) - target = 0 on [r_min, r_max]
        while (r_hi < r_max && m_hi < target) {
            r_lo = r_hi;
            m_lo = m_hi;
            r_hi = std::min(r_hi * 2, r_max);
            m_hi = medium.mass(r_hi);
        }
        ...
        if (m_hi <= m_lo) {
            return r_hi * (1 - beta) / (beta * con::c);
        }

        for (size_t it = 0; it < 80; ++it) {
            const Real r_mid = 0.5 * (r_lo + r_hi);
            const Real m_mid = medium.mass(r_mid);
            if (m_mid < target) {
                r_lo = r_mid;
                m_lo = m_mid;
            } else {
