...
BOOST_AUTO_TEST_CASE(wind_general_k_density_powerlaw) {
    // For n0=inf and n_ism=0, Wind reduces to rho = A / r^k for any k.
    const Real A_star = 0.6;
    const Real k = 1.5;
    Wind wind(A_star, 0, con::inf, k);
    const Real A = A_star * 5e11 * unit::g / unit::cm * std::pow(1e17 * unit::cm, k - 2);

    const Real r = 2e18;
    const Real expected = A / std::pow(r, k);
    BOOST_CHECK_CLOSE(wind.rho(0, 0, r), expected, 1e-8);
}

BOOST_AUTO_TEST_CASE(wind_general_k_mass_powerlaw) {
    // For n0=inf and n_ism=0, mass per solid angle is A*r^(3-k)/(3-k) for k<3.
    const Real A_star = 1.2;
    const Real k = 1.2;
    Wind wind(A_star, 0, con::inf, k);
    const Real A = A_star * 5e11 * unit::g / unit::cm * std::pow(1e17 * unit::cm, k - 2);

    const Real r = 8e17;
    const Real expected = A * std::pow(r, 3 - k) / (3 - k);
    BOOST_CHECK_CLOSE(wind.mass(r), expected, 1e-8);
}
