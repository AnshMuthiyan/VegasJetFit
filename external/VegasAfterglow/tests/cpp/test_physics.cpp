...
BOOST_AUTO_TEST_CASE(powerlaw_dec_radius_general_k) {
    // Verify explicit k-dependent deceleration scalings used for n(r)=n0*(r/r0)^(-k).
    const Real n0 = 0.3;
    const Real k = 1.4;
    const Real r0 = 1e17 * unit::cm;

    const Real expected_thin = std::pow(
        (3 - k) * E_iso / (4.0 * con::pi * con::mp * con::c2 * n0 * std::pow(r0, k) * Gamma0 * Gamma0),
        1.0 / (3 - k));
    const Real expected_thick = std::pow(
        (3 - k) * E_iso * engine_dura * con::c / (4.0 * con::pi * con::mp * con::c2 * n0 * std::pow(r0, k)),
        1.0 / (4 - k));

    const Real thin = physics::scales::thin_shell_dec_radius(E_iso, n0, Gamma0, k, r0);
    const Real thick = physics::scales::thick_shell_dec_radius(E_iso, n0, engine_dura, k, r0);

    BOOST_CHECK_CLOSE(thin, expected_thin, 1e-10);
    BOOST_CHECK_CLOSE(thick, expected_thick, 1e-10);
    BOOST_CHECK(std::isfinite(thin));
    BOOST_CHECK(std::isfinite(thick));
    BOOST_CHECK_GT(thin, 0.0);
    BOOST_CHECK_GT(thick, 0.0);
}
