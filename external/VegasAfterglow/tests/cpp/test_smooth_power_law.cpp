...
namespace {
    ...
    Real ref_s12_slow(Real k, Real p) {
        return 1.84 - 0.040 * k - (0.40 - 0.010 * k) * p;
    }

    Real ref_s23_slow(Real k, Real p) {
        return 1.15 - 0.125 * k - (0.06 - 0.015 * k) * p;
    }

    Real ref_s12_mac(Real k, Real p) {
        return 1.47 - 0.11 * k - (0.21 - 0.015 * k) * p;
    }

    Real ref_s23_fast(Real k, Real p) {
        return 3.34 + 0.17 * k - (0.82 + 0.035 * k) * p;
    }

    Real reference_ampy_flux(Real nu, Real nu_a, Real nu_m, Real nu_c, Real I_nu_max, Real p, Real k, Real nu_M,
                             bool smooth_fast_to_slow_transition = false) {
        const bool slow = nu_m < nu_c;
        const bool mac = slow && (nu_m < nu_a);
        const bool cam = (!slow) && (nu_c < nu_a);

        const Real nu12 = slow ? (mac ? nu_a : nu_m) : (cam ? nu_a : nu_c);
        const Real nu23 = slow ? nu_c : nu_m;

        Real b1 = 1.0 / 3.0;
        Real b2 = slow ? (1.0 - p) / 2.0 : -0.5;
        const Real b3 = -p / 2.0;
        if (mac) {
            b1 = 2.5;
        }
        ...
        Real s12 = slow ? ref_s12_slow(k, p) : 0.597;
        Real s23 = slow ? ref_s23_slow(k, p) : ref_s23_fast(k, p);
        if (mac) {
            s12 = ref_s12_mac(k, p);
        }
        if (cam) {
        ...
        if (smooth_fast_to_slow_transition) {
            ...
            const Real q12 = -s12 * (b3 - b1);
            const Real q23 = -s23 * (b3 - b1);
            s12 = 0.597 + (ref_s12_slow(k, p) - 0.597) / (1.0 + std::pow(nu_ratio, q12));
            s23 = ref_s23_fast(k, p) + (ref_s23_slow(k, p) - ref_s23_fast(k, p)) / (1.0 + std::pow(nu_ratio, q23));
            b2a = -0.5 + (((1.0 - p) / 2.0) + 0.5) / (1.0 + std::pow(nu_ratio, q12));
            b2b = -0.5 + (((1.0 - p) / 2.0) + 0.5) / (1.0 + std::pow(nu_ratio, q23));
        }

        const Real x12 = nu / nu12;
        const Real x23 = nu / nu23;
        const Real term1 =
            std::pow(std::pow(x12, -s12 * (b1 - b2a)) + 1.0, s23 / s12) * std::pow(x12, -s23 * b2a);
        const Real term2 = std::pow(nu23 / nu12, -s23 * b2b) * std::pow(x23, -s23 * b3);

        Real flux = I_nu_max * std::pow(term1 + term2, -1.0 / s23) * std::exp(-nu / nu_M);
        if (mac) {
            flux *= std::pow(nu_a / nu_m, b2a);
        }
        if (cam && nu > nu_a) {
            flux *= (1.0 / 3.0) * std::sqrt(nu_c / nu_a);
        }
        return flux;
    }
...
// ---------------------------------------------------------------------------
// 4b. matches_ampy_reference_formula
// ---------------------------------------------------------------------------
BOOST_AUTO_TEST_CASE(matches_ampy_reference_formula) {
    struct Case {
        Real nu_a;
        Real nu_m;
        Real nu_c;
    ...
    const Case cases[] = {
        {1e8, 1e12, 1e16, 2.0},   // regime 1
        {1e12, 1e8, 1e16, 1.5},   // regime 2
        {1e8, 1e16, 1e12, 0.0},   // regime 3
        {1e12, 1e16, 1e8, -1.0},  // regime 4
        {1e16, 1e8, 1e12, 2.0},   // regime 5
        {1e16, 1e12, 1e8, 2.0},   // regime 6
    };

    for (const auto& tc : cases) {
        auto ph = make_photon(tc.nu_a, tc.nu_m, tc.nu_c, 1e30, 1e-20, kP);
        ph.k_eff = tc.k_eff;
        ph.build();

        const bool slow = ph.nu_m < ph.nu_c;
        const bool mac = slow && (ph.nu_m < ph.nu_a);
        const bool cam = (!slow) && (ph.nu_c < ph.nu_a);
        const Real nu12 = slow ? (mac ? ph.nu_a : ph.nu_m) : (cam ? ph.nu_a : ph.nu_c);
        const Real nu23 = slow ? ph.nu_c : ph.nu_m;
        const Real nu_lo = std::min(ph.nu_a, std::min(ph.nu_m, ph.nu_c));
        const Real nu_mid_lo = std::min(nu12, nu23);
        const Real nu_mid_hi = std::max(nu12, nu23);
        const Real probes[] = {
            0.3 * nu_lo,
            0.8 * nu_mid_lo,
            1.2 * nu_mid_lo,
        ...
        for (Real nu : probes) {
            const Real compiled = ph.compute_I_nu(nu);
            const Real reference = reference_ampy_flux(nu, ph.nu_a, ph.nu_m, ph.nu_c, ph.I_nu_max, ph.p, ph.k_eff,
                                                       ph.nu_M);
            BOOST_CHECK_CLOSE(compiled, reference, 1e-6);
        }
    }
}

// ---------------------------------------------------------------------------
// 4c. matches_ampy_reference_formula_with_fast_to_slow_transition_smoothing
// ---------------------------------------------------------------------------
BOOST_AUTO_TEST_CASE(matches_ampy_reference_formula_with_fast_to_slow_transition_smoothing) {
    struct Case {
        Real nu_a;
        Real nu_m;
        Real nu_c;
    ...
    const Case cases[] = {
        {1e8, 1e14, 1e12, 1.0},   // still fast cooling during transition
        {1e8, 1e12, 1e14, 1.0},   // already slow cooling during transition
        {1e13, 1e14, 1e12, 0.0},  // fast cooling with nu_a above the first break
        {1e13, 1e12, 1e14, 0.0},  // slow cooling with nu_a above the first break
    };

    for (const auto& tc : cases) {
        auto ph = make_photon(tc.nu_a, tc.nu_m, tc.nu_c, 1e30, 1e-20, kP);
        ph.k_eff = tc.k_eff;
        ph.smooth_fast_to_slow_transition = true;
        ph.build();
        ...
        for (Real nu : probes) {
            ...
            const Real reference =
                reference_ampy_flux(nu, ph.nu_a, ph.nu_m, ph.nu_c, ph.I_nu_max, ph.p, ph.k_eff, ph.nu_M, true);
            BOOST_CHECK_CLOSE(compiled, reference, 1e-6);
        }
    }
}
...
// ---------------------------------------------------------------------------
// 6b. medium_slope_changes_smoothing
// ---------------------------------------------------------------------------
BOOST_AUTO_TEST_CASE(medium_slope_changes_smoothing) {
    // The compiled smoother should respond to the local medium slope k_eff.
    // Use a frequency close to the lower break where the smoothing parameter
    // matters but the spectrum stays finite and positive.
    auto ph_ism_like = make_photon(1e10, 1e14, 1e16);
    auto ph_wind_like = make_photon(1e10, 1e14, 1e16);
    ph_ism_like.k_eff = 0.0;
    ph_wind_like.k_eff = 2.0;
    ph_ism_like.build();
    ph_wind_like.build();

    const Real nu_probe = 0.8 * ph_ism_like.nu_m;
    const Real I_ism_like = ph_ism_like.compute_I_nu(nu_probe);
    const Real I_wind_like = ph_wind_like.compute_I_nu(nu_probe);

    BOOST_CHECK_GT(I_ism_like, 0.0);
    BOOST_CHECK_GT(I_wind_like, 0.0);
    BOOST_CHECK(std::isfinite(I_ism_like));
    BOOST_CHECK(std::isfinite(I_wind_like));
    BOOST_CHECK_NE(I_ism_like, I_wind_like);
}
