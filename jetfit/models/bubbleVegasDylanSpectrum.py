...
class BubbleVegasDylanSpectrumModel(BubbleVegasModel):
    """
    Bubble VegasAfterglow adapter with Dylan/GS02 spectral smoothing.

    The dynamics and characteristic frequencies are still taken from
    VegasAfterglow through ``BubbleVegasModel``. Only the analytic spectral
    break-shape prescription is replaced with the same JetFit smoothing used by
    ``powerlawVegasDylanSpectrumModel``.
    """

    def _interpolated_details(self, t):
        t_sec = np.atleast_1d(t).astype(float) * 86400.0
