...
export CONFIG_DIR="${CONFIG_DIR:-$VEGAS_DIR/structured_jet_finalprod_unseeded_configs_active}"
export RUN_TAG="${RUN_TAG:-structjet_thetav_thetac_electronp_unseeded_finalprod_10temp_5000x5000_v1}"
if [ -z "${RESULTS_NAME_TEMPLATE:-}" ]; then
  # Keep this as a literal template for jwk_run_thesis_reproduction_event.sh.
...
# Keep the remote launch guarded by the same smoke-test logic used locally.
