#!/usr/bin/env bash
# Guarded workstation launcher for the 2026-09-04 all-UVOIR GRB 080319B refit.
set -euo pipefail
...
verify_sha256 0119aec426c16aef7d0c13660949126bd3a7678593b2d61ae43b0a2eb7e87588 "$INITIAL_POSITIONS"
...
with np.load(seed_path, allow_pickle=False) as seed:
    ...
    if not source.endswith("080319B_core_logangle_powerlawcsm_kminus10to3_finalfinal_sthawed_n17upper15_5temp_1000x5000_v1"):
        raise SystemExit(f"initializer has the wrong source posterior: {source}")
    if str(seed["ridge_parameter"]) or str(seed["ridge_coupled_parameter"]):
        raise SystemExit("all-UVOIR initializer must be an unmodified posterior-cloud draw")
...
echo "080319B all-UVOIR n17-upper15 workstation preflight guard passed on $HOST_SHORT with $WORKERS workers"
exec env \
  RUN_TAG="$RUN_TAG" \
