#!/usr/bin/env bash
# Guarded launcher for the GRB 130612A n17-upper25 continuation.
set -euo pipefail
...
[ "$EVENT" = "130612A" ] || { echo "ERROR: this runner is only for 130612A" >&2; exit 2; }
case "$HOST_SHORT" in
...
verify_sha256 967b035cfada68cca062642cad8eb0ee0c09ede01f3ef7f67fc793b252409629 "$CONFIG_DIR/130612A.toml"
verify_sha256 b3f598bf81f2ed1370cf8e66d859b56e6e50248f94cb8cf13e9cce47de89483f "$OBS_CSV_OVERRIDE"
verify_sha256 21ad71f9f74db30569a0dd10c1ab851653fea276c666eb90dfde5b219f7ec983 "$INITIAL_POSITIONS"
...
"$ROOT/.venv/bin/python" - "$CONFIG_DIR/130612A.toml" "$INITIAL_POSITIONS" "$OBS_CSV_OVERRIDE" <<'PY'
import sys
import tomllib
...
with np.load(seed_path, allow_pickle=False) as seed:
    ...
    if positions.shape != (5, 100, 15) or not np.isfinite(positions).all():
        raise SystemExit(f"invalid posterior cloud: shape={positions.shape}")
    source = str(seed["source_results"])
    if not source.endswith("130612A_core_logangle_powerlawcsm_finalfinal_sthawed_n17upper15_audit_5temp_1000x5000_v1"):
...
if len(obs) != 51 or "Include" not in obs or not (obs["Include"] == 1).all():
    raise SystemExit("observation override does not include all 51 authoritative rows")
PY

echo "130612A n17-upper25 workstation guard passed on $HOST_SHORT with $WORKERS workers"
exec env \
  RUN_TAG="$RUN_TAG" \
