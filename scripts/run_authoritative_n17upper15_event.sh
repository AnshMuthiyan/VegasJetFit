#!/usr/bin/env bash
# Guarded launcher for authoritative posterior-cloud n17 boundary follow-ups.
set -euo pipefail
...
INITIAL_POSITIONS="$VJF/initial_positions/authoritative_n17upper15_5temp/${EVENT}.npz"
OBS_CSV_OVERRIDE="$VJF/obs_overrides/${EVENT}_authoritative_n17upper15_20260904.csv"
...
case "$EVENT" in
  050525A)
    CONFIG_SHA="ac31dee6dc15a21e2d7556113162a6f1257d9b51e51da9b15e91da7c85fae748"
    OBS_SHA="d28d0215d1c57cf6be9be58aa1a4155cba3f1f948ef856b9234af57e07337f6f"
    SEED_SHA="1cdbdc224628e8c8566c3e617e958c7165f3fe5ac99259953d81d1bd2324132e"
    SOURCE_SUFFIX="050525A_core_logangle_powerlawcsm_kminus10to3_finalfinal_sthawed_resolution_refinement_5temp_500x3000_v1"
    EXPECTED_PARAMETERS=23
    EXPECTED_ROWS=110
    ;;
  050922C)
    CONFIG_SHA="27595d8950bf5888a93a201acab52c9bb156fe8142fdb88cf60cc5d9188c9b75"
    OBS_SHA="b50e66aa81bd35523f35ef574a43f314fe008b8fb1fe366b29a3c7e84853d258"
    SEED_SHA="e41fdf53ead2acfc8e77cb96eedcadf5b70c26eb595942bc93ebe3f069ae5421"
    SOURCE_SUFFIX="050922C_core_logangle_powerlawcsm_kminus10to3_finalfinal_sthawed_resolution_refinement_5temp_500x3000_v1"
    EXPECTED_PARAMETERS=15
    EXPECTED_ROWS=255
    ;;
  130612A)
    CONFIG_SHA="03ea5cd080e71250f36b341ec9fd08eab168ec9953808b4428211b28d71d685f"
    OBS_SHA="b3f598bf81f2ed1370cf8e66d859b56e6e50248f94cb8cf13e9cce47de89483f"
    SEED_SHA="7a94c9a01692d01ed66e06962c7b5a7c52a35ad66026ab5bd3eeb0cea0ad583e"
    SOURCE_SUFFIX="130612A_core_logangle_powerlawcsm_kminus10to3_finalfinal_sthawed_fine_5temp_500x3000_v1"
    EXPECTED_PARAMETERS=15
    EXPECTED_ROWS=51
    ;;
  *) echo "ERROR: unsupported event $EVENT" >&2; exit 2 ;;
esac
...
case "$HOST_SHORT" in
  ...
  lyra)
    [ "${PREFLIGHT_ONLY:-0}" = "1" ] || {
      echo "ERROR: Lyra is permitted only for local preflight" >&2
      exit 2
    }
    EXPECTED_WORKERS=8
    ;;
...
verify_sha256 "$CONFIG_SHA" "$CONFIG_DIR/${EVENT}.toml"
verify_sha256 "$OBS_SHA" "$OBS_CSV_OVERRIDE"
verify_sha256 "$SEED_SHA" "$INITIAL_POSITIONS"
...
"$ROOT/.venv/bin/python" - "$CONFIG_DIR/${EVENT}.toml" "$INITIAL_POSITIONS" "$OBS_CSV_OVERRIDE" "$SOURCE_SUFFIX" "$EXPECTED_PARAMETERS" "$EXPECTED_ROWS" <<'PY'
import sys
import tomllib
...
model_path, seed_path, obs_path, source_suffix, expected_parameters, expected_rows = sys.argv[1:]
...
with np.load(seed_path, allow_pickle=False) as seed:
    ...
    expected_shape = (5, 100, int(expected_parameters))
    if positions.shape != expected_shape or not np.isfinite(positions).all():
        raise SystemExit(f"invalid posterior cloud: shape={positions.shape}, expected={expected_shape}")
    if not str(seed["source_results"]).endswith(source_suffix):
        raise SystemExit("initializer has the wrong source posterior")
    ...
    if float(seed["ridge_coupled_factor"]) != -1.0 or float(seed["ridge_max_shift"]) != 4.0:
        raise SystemExit("initializer has the wrong ridge controls")
    n_index = names.index("n017")
    if positions[:, :, n_index].min() <= -6.0 or positions[:, :, n_index].max() >= 15.0:
        raise SystemExit("initializer contains n017 values outside the approved prior")

obs = pd.read_csv(obs_path)
if len(obs) != int(expected_rows):
    raise SystemExit(f"unexpected observation row count: {len(obs)}")
if "Include" in obs and not (obs["Include"] == 1).all():
    raise SystemExit("authoritative observation override contains excluded rows")
PY

echo "$EVENT authoritative n17-upper15 guard passed on $HOST_SHORT with $WORKERS workers"
exec env \
  RUN_TAG="$RUN_TAG" \
