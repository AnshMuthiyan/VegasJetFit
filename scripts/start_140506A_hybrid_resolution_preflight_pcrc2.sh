#!/usr/bin/env bash
# Launch only the guarded hybrid-resolution resource preflight on PCRC-2.
set -euo pipefail
...
WORKERS=4
RUN_TAG="core_logangle_powerlawcsm_kminus10to3_finalfinal_sthawed_hybrid_5temp_500x3000_v1"
REPORT="$VJF/reports/core_logangle_powerlaw_140506A_finalfinal_sthawed_hybrid_5temp_500x3000_campaign"
...
SESSION="grb_140506A_hybrid_preflight"
SSH=(ssh -x -o ForwardX11=no -o BatchMode=yes -o ConnectTimeout=15)

"${SSH[@]}" "$HOST" "mkdir -p '$REPORT'"
rsync -az -e "ssh -x -o ForwardX11=no -o BatchMode=yes -o ConnectTimeout=15" \
  "$REPORT/" "$HOST:$REPORT/"
rsync -az -e "ssh -x -o ForwardX11=no -o BatchMode=yes -o ConnectTimeout=15" \
  "$RUNNER" "$VJF/jwk_run_thesis_reproduction_event.sh" "$HOST:$VJF/scripts/"
rsync -az -e "ssh -x -o ForwardX11=no -o BatchMode=yes -o ConnectTimeout=15" \
  "$CONFIG_DIR/" "$HOST:$CONFIG_DIR/"
rsync -az -e "ssh -x -o ForwardX11=no -o BatchMode=yes -o ConnectTimeout=15" \
  "$MCMC_SETTINGS" "$HOST:$VJF/Ansh_Run/"
rsync -az -e "ssh -x -o ForwardX11=no -o BatchMode=yes -o ConnectTimeout=15" \
  "$INITIAL_POSITIONS" "$HOST:$INITIAL_POSITIONS"

command="cd '$VJF' && EVENT='$EVENT' WORKERS='$WORKERS' RUN_TAG='$RUN_TAG' DISPATCH_MANIFEST='$REPORT/dispatch_manifest.csv' CONFIG_DIR='$CONFIG_DIR' MCMC_SETTINGS='$MCMC_SETTINGS' INITIAL_POSITIONS='$INITIAL_POSITIONS' FORCE_FRESH=1 ENABLE_PREFLIGHT=1 PREFLIGHT_ONLY=1 PREFLIGHT_BURN_LENGTH=1 PREFLIGHT_RUN_LENGTH=1 CLEAN_INCOMPLETE=1 SKIP_COMPLETED=0 bash '$RUNNER'"
