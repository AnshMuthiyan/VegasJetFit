#!/usr/bin/env bash
# Publish the queued 130612A fine-resolution continuation from Lyra only.
set -euo pipefail
...
SESSION="postprocess_130612A_finalfine_500x3000_lyra"
LOG="$VJF/logs/postprocess_130612A_finalfine_500x3000_lyra.log"
...
tmux new-session -d -s "$SESSION" \
  "cd '$VJF' && exec env MANIFEST='$MANIFEST' RUN_TAG='$RUN_TAG' CAMPAIGN='$CAMPAIGN' LOG='$LOG' POLL_SECONDS=300 DENSITY_PROFILE_SAMPLES=0 PRODUCT_WORKERS=8 MINIMIZER_MAX_WALKERS=8 MINIMIZER_WORKERS=8 RESOLUTION_CONVERGENCE=1 SAMPLER_LABEL='100walkers_500burn_3000run_5temps_posterior_cloud_fine_resolution' CAMPAIGN_SUMMARY_TITLE='GRB 130612A Fine-Resolution Final-Final Continuation' CAMPAIGN_SUMMARY_PURPOSE='Full posterior-cloud seeded resolution refinement at the measured next coupled VegasAfterglow grid.' bash '$WATCHER'"
echo "Started 130612A fine-resolution postprocessor: tmux attach -t $SESSION"
