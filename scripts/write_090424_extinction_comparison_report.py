...
def build_document(output_dir: Path, payload: dict) -> str:
    ...
    lines = [
        ...
        r"\clearpage",
        r"\section{Fitted Multiwavelength Light Curves}",
        r"\begin{figure}[H]",
        r"\centering",
        r"\includegraphics[width=0.96\textwidth,height=0.78\textheight,keepaspectratio]{090424_ccm_vs_trotter_light_curve_comparison.pdf}",
        r"\caption{Observed multiwavelength light curves and the retained best-fit predictions for the CCM and Trotter source-frame extinction models. The observations are shown on their original calibration scale, while each model curve includes that fit's retained calibration offsets and therefore reproduces the prediction evaluated by the likelihood. The lower panel gives the full fitted-curve ratio, including changes in the shared afterglow parameters as well as the extinction law. A value of zero denotes identical fluxes. Open gray points were excluded from both fits.}",
        r"\end{figure}",
        r"\clearpage",
